...
import { BadText, GenericInput, GenericLabel, GoodText, LoginModal, ModalButton, ModalTitle } from "src/pages/Landing/style";
      ...
      {isOpen ? (
        <Dialog close={toggleOpen} modalSize="30">
          <LoginModal>
            ...
            {isNotMatching && (
              <BadText>Passwords do not match.</BadText>
            )}
            {loadingStatus===UserLoadingStatusTypes.PUT_USER_UPDATE_COMPLETED && (
              <GoodText>Your details has been updated.</GoodText>
            )}
            <GenericLabel>New Name</GenericLabel>
            <GenericInput placeholder="New name" type="text" onChange={(e) => setNewName(e.target.value)} />
            <GenericLabel>New Password</GenericLabel>
            <GenericInput placeholder="New password" type="text" onChange={(e) => setNewPassword(e.target.value)} />
            <GenericLabel>Confirm New Password</GenericLabel>
            <GenericInput placeholder="Confirm New password" type="text" onChange={(e) => setConfirmNewPassword(e.target.value)} />
