  ...
  setupRoutes(): Router {
    return Router()
      ...
      .get(
        "/articles/:articleId",
        async (req: Request, res: Response, next: NextFunction) => {
          this.logger.info(`Received /articles/specific request`);
          try {
            const articleId: string = req.params.articleId;
            const result = await this.articleService.getSpecificArticle(
              articleId
            );
            this.logger.info(`Responding to client in GET /articles/specific`);
            return res.status(200).json(result);
          } catch (err: any) {
            this.logger.warn(
